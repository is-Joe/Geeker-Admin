  ...
  actions: {
    ...
    async closeTabsOnSide(
      ...
      if (currentIndex !== -1) {
        const range =
          type === "left"
            ...
            : [
                currentIndex + 1,
                this.tabsMenuList.length
              ];
        this.tabsMenuList =
          this.tabsMenuList.filter(
            (item, index) => {
              return (
                index < range[0] ||
                index >= range[1] ||
      ...
      keepAliveStore.setKeepAliveName(
        this.tabsMenuList.map(item => item.name)
      );
    },
    // Close MultipleTab
